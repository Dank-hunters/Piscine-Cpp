...
void ClapTrap::beRepaired(unsigned int amount){ 
    ...
    else if (this->Hit_P < 10)
    {
        std::cout << "ClapTrap " << this->Name << " repaired himself from " << amount << " Hit point" << std::endl;
        this->Hit_P += amount;
        if (this->Hit_P > 10)
        {
               std::cout << "ClapTrap " << this->Name << " can't have more than 10 HP, so he just have 10 HP now" << std::endl;
                this->Hit_P = 10;
        }
        else 
